...
def get_dataset(dataset, data_path='', batch_size=1, subset="imagenette", args=None, im=224):
    ...
    if dataset == 'CIFAR10':
        ...
        train_pipeline_cfg = [
            ...
            dict(type='Resize', scale=(im, im), keep_ratio=False),
        ...
        ann_class = f'./data/ucf101/annotations/classInd.txt'
        ...
        dst_train = DATASETS.build(train_dataset_cfg)
        # dst_test = DATASETS.build(test_dataset_cfg)
        dst_test = None
        class_names = None
        # with open(ann_class, 'r') as f:
        #     classes = f.readlines()
...
def build_original_dataset(channel, num_classes, dst_train, class_map):
    images_all = []
    labels_all = []
    ...
    #     # print('real images channel %d, mean = %.4f, std = %.4f'%(ch, torch.mean(images_all[:, :, ch, :, :].float()), torch.std(images_all[:, :, ch, :, :].float())))
    #     print('real images channel %d, mean = %.4f, std = %.4f'%(ch, torch.mean(images_all[:, ch, :, :].float()), torch.std(images_all[:, ch, :, :].float())))
